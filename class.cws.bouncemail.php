...
define('CWSBM_VERBOSE_QUIET',  0); // means no output at all
define('CWSBM_VERBOSE_SIMPLE', 1); // means only output simple report
define('CWSBM_VERBOSE_REPORT', 2); // means output a detail report
define('CWSBM_VERBOSE_DEBUG',  3); // means output detail report as well as debug info.

define('CWSBM_CERT_NOVALIDATE', 'novalidate-cert'); // do not validate certificates from TLS/SSL server
define('CWSBM_CERT_VALIDATE',   'validate-cert');   // validate certificates from TLS/SSL server (default behavior)

require_once('class.cws.bouncemail.rules.php');

class CwsBounceMail extends CwsBounceMailRules
{
    /**
     * CwsBounceMail version.
     * @var string
     */
    public $version = "1.0";
     ...
     * default CWSBM_CERT_NOVALIDATE
     * @var string
     */
    public $cert = CWSBM_CERT_NOVALIDATE;
     ...
     * default CWSBM_VERBOSE_SIMPLE
     * @var int
     */
    public $debug_verbose = CWSBM_VERBOSE_SIMPLE;
     ...
     */
    private function output($msg=false, $verbose_level=CWSBM_VERBOSE_SIMPLE, $newline=true)
    {
    ...
    {
    	$this->output('<h2>Init openRemote</h2>', CWSBM_VERBOSE_SIMPLE, false);
    ...
    {
    	$this->output('<h2>Init openLocal</h2>', CWSBM_VERBOSE_SIMPLE, false);
    ...
    {
    	$this->output('<h2>Init processMailbox</h2>', CWSBM_VERBOSE_SIMPLE, false);
        ...
        for ($x = 1; $x <= $c_fetched; $x++) {
            if ($this->use_fetchstructure) {
                ...
                if ($structure->type == 1 && $structure->ifsubtype && $structure->subtype == $this->_subtype_name && $structure->ifparameters && $this->isParameter($structure->parameters)) {
                    ...
                    $this->output('<strong>Msg #' .    $x . '</strong> is not a standard DSN message', CWSBM_VERBOSE_REPORT);
                    if ($this->debug_verbose == CWSBM_VERBOSE_DEBUG) {
                        if ($structure->ifdescription) {
                            $this->output('<strong>Content-Type:</strong> {' . $structure->description . '}', CWSBM_VERBOSE_DEBUG);
                        } else {
                            $this->output('<strong>Content-Type:</strong> unsupported', CWSBM_VERBOSE_DEBUG);
                        }
                    }
                ...
                if (preg_match('#Content-Type:((?:[^\n]|\n[\t ])+)(?:\n[^\t ]|$)#is', $header, $match)) {
                    if (preg_match('#multipart/' . $this->_subtype_name . '#is', $match[1]) && preg_match('#' . $this->_report_type . '=["\']?' . $this->_dsn_name . '["\']?#is', $match[1])) {
                        ...
                        $this->output('<strong>Msg #' . $x . '</strong> is not a standard DSN message', CWSBM_VERBOSE_REPORT);
                        if ($this->debug_verbose == CWSBM_VERBOSE_DEBUG) {
                            $this->output('<strong>Content-Type:</strong> {' . $match[1] . '}', CWSBM_VERBOSE_DEBUG);
                        }
                        $processed = $this->processBounce($x, 'BODY');
                    }
                } else {
                    // didn't get content-type header
                    $this->output('<strong>Msg #' . $x . '</strong> is not a well-formatted MIME mail, missing Content-Type', CWSBM_VERBOSE_REPORT);
                    if ($this->debug_verbose == CWSBM_VERBOSE_DEBUG) {
                        $this->output('<strong>Headers:</strong> ' . $this->_newline . $header . $this->_newline, CWSBM_VERBOSE_DEBUG);
                    }
                    $processed = $this->processBounce($x, 'BODY');
                }
            }
        ...
        $this->output('<h2>End of process</h2>', CWSBM_VERBOSE_SIMPLE, false);
        ...
        $this->output('<h2>$result</h2>', CWSBM_VERBOSE_REPORT, false);
        if (!empty($this->result)) {
        	$this->output('<h3>resume</h3>', CWSBM_VERBOSE_REPORT, false);
        	foreach ($this->result['resume'] as $key => $value) {
        		$this->output('<strong>' . $key . '</strong> => ' . $value, CWSBM_VERBOSE_REPORT);
        	}
        	$this->output('<h3>msgs</h3>', CWSBM_VERBOSE_REPORT, false);
        	if (!empty($this->result['msgs'])) {
        		foreach ($this->result['msgs'] as $msg) {
        			foreach ($msg as $key => $value) {
        				$this->output('<strong>' . $key . '</strong> => ' . $value, CWSBM_VERBOSE_REPORT);
        			}
        			$this->output('&nbsp;', CWSBM_VERBOSE_REPORT);
        		}
        	} else {
        		$this->output('empty', CWSBM_VERBOSE_REPORT);
        	}
        }
    ...
    private function processBounce($pos, $type)
    {
        ...
        if ($type == 'DSN') {
            ...
            $rulesResult = $this->processDSNRules($pos, $dsn_msg, $dsn_report, $this->debug_verbose == CWSBM_VERBOSE_DEBUG);
        } elseif ($type == 'BODY') {
            ...
            switch ($structure->type) {
                case 0:
                    ...
                    $rulesResult = $this->processBodyRules($pos, $body, $structure, $this->debug_verbose == CWSBM_VERBOSE_DEBUG);
                    break;
                case 1:
                ...
                default:
                    ...
                    $this->output('Msg #' . $pos . ' is unsupported Content-Type:' . $structure->type, CWSBM_VERBOSE_REPORT);
                    return false;
            }
        } else {
            // internal error
