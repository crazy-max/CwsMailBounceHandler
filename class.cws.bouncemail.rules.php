 ...
 * CwsBounceMail
 *
 * CwsBounceMail is a PHP toolkit : (CwsBounceMail and CwsBounceMailRules)
 ...
 *
 * @package CwsBounceMail
 * @author Cr@zy
 * @copyright 2013, Cr@zy
 * @license GPL licensed
 * @version 1.0
 * @link https://github.com/crazy-max/CwsBounceMail
 *
 */

define('CWSBM_BOUNCE_ANTISPAM',			'antispam');
define('CWSBM_BOUNCE_AUTOREPLY',		'autoreply');
define('CWSBM_BOUNCE_CONCURRENT',		'concurrent');
define('CWSBM_BOUNCE_CONTENT_REJECT',	'content_reject');
define('CWSBM_BOUNCE_COMMAND_REJECT',	'command_reject');
define('CWSBM_BOUNCE_INTERNAL_ERROR',	'internal_error');
define('CWSBM_BOUNCE_DEFER',			'defer');
define('CWSBM_BOUNCE_DELAYED',			'delayed');
define('CWSBM_BOUNCE_DNS_LOOP',			'dns_loop');
define('CWSBM_BOUNCE_DNS_UNKNOWN',		'dns_unknown');
define('CWSBM_BOUNCE_FULL',				'full');
define('CWSBM_BOUNCE_INACTIVE',			'inactive');
define('CWSBM_BOUNCE_LATIN_ONLY',		'latin_only');
define('CWSBM_BOUNCE_OTHER',			'other');
define('CWSBM_BOUNCE_OVERSIZE',			'oversize');
define('CWSBM_BOUNCE_OUT_OF_OFFICE',	'outofoffice');
define('CWSBM_BOUNCE_UNKNOWN',			'unknown');
define('CWSBM_BOUNCE_UNRECOGNIZED',		'unrecognized');
define('CWSBM_BOUNCE_USER_REJECT',		'user_reject');
define('CWSBM_BOUNCE_WARNING',			'warning');

define('CWSBM_BOUNCE_TYPE_NULL',		null);
define('CWSBM_BOUNCE_TYPE_BLOCKED',		'blocked');
define('CWSBM_BOUNCE_TYPE_AUTOREPLY',	'autoreply');
define('CWSBM_BOUNCE_TYPE_SOFT',		'soft');
define('CWSBM_BOUNCE_TYPE_HARD',		'hard');
define('CWSBM_BOUNCE_TYPE_TEMPORARY',	'temporary');
define('CWSBM_BOUNCE_TYPE_GENERIC',		'generic');

class CwsBounceMailRules
{
	/**
	...
	/**
	 * Defines based process result
	 */
	private $_based_result = array(
		'email'       => '',
		'bounce_type' => null,
		'remove'      => 0,
	...
	(
		CWSBM_BOUNCE_ANTISPAM => array
		(
			'remove'		=> false,
			'bounce_type'	=> CWSBM_BOUNCE_TYPE_BLOCKED,
		),
		CWSBM_BOUNCE_AUTOREPLY => array
		(
			'remove'		=> false,
			'bounce_type'	=> CWSBM_BOUNCE_TYPE_AUTOREPLY,
		),
		CWSBM_BOUNCE_CONCURRENT => array
		(
			'remove'		=> false,
			'bounce_type'	=> CWSBM_BOUNCE_TYPE_SOFT,
		),
		CWSBM_BOUNCE_CONTENT_REJECT => array
		(
			'remove'		=> false,
			'bounce_type'	=> CWSBM_BOUNCE_TYPE_SOFT,
		),
		CWSBM_BOUNCE_COMMAND_REJECT => array
		(
			'remove'		=> true,
			'bounce_type'	=> CWSBM_BOUNCE_TYPE_HARD,
		),
		CWSBM_BOUNCE_INTERNAL_ERROR => array
		(
			'remove'		=> false,
			'bounce_type'	=> CWSBM_BOUNCE_TYPE_TEMPORARY,
		),
		CWSBM_BOUNCE_DEFER => array
		(
			'remove'		=> false,
			'bounce_type'	=> CWSBM_BOUNCE_TYPE_SOFT,
		),
		CWSBM_BOUNCE_DELAYED => array
		(
			'remove'		=> false,
			'bounce_type'	=> CWSBM_BOUNCE_TYPE_TEMPORARY,
		),
		CWSBM_BOUNCE_DNS_LOOP => array
		(
			'remove'		=> true,
			'bounce_type'	=> CWSBM_BOUNCE_TYPE_HARD,
		),
		CWSBM_BOUNCE_DNS_UNKNOWN => array
		(
			'remove'		=> true,
			'bounce_type'	=> CWSBM_BOUNCE_TYPE_HARD,
		),
		CWSBM_BOUNCE_FULL => array
		(
			'remove'		=> false,
			'bounce_type'	=> CWSBM_BOUNCE_TYPE_SOFT,
		),
		CWSBM_BOUNCE_INACTIVE => array
		(
			'remove'		=> true,
			'bounce_type'	=> CWSBM_BOUNCE_TYPE_HARD,
		),
		CWSBM_BOUNCE_LATIN_ONLY => array
		(
			'remove'		=> false,
			'bounce_type'	=> CWSBM_BOUNCE_TYPE_SOFT,
		),
		CWSBM_BOUNCE_OTHER => array
		(
			'remove'		=> true,
			'bounce_type'	=> CWSBM_BOUNCE_TYPE_GENERIC,
		),
		CWSBM_BOUNCE_OVERSIZE => array
		(
			'remove'		=> false,
			'bounce_type'	=> CWSBM_BOUNCE_TYPE_SOFT,
		),
		CWSBM_BOUNCE_OUT_OF_OFFICE => array
		(
			'remove'		=> false,
			'bounce_type'	=> CWSBM_BOUNCE_TYPE_SOFT,
		),
		CWSBM_BOUNCE_UNKNOWN => array
		(
			'remove'		=> true,
			'bounce_type'	=> CWSBM_BOUNCE_TYPE_HARD,
		),
		CWSBM_BOUNCE_UNRECOGNIZED => array
		(
			'remove'		=> false,
			'bounce_type'	=> CWSBM_BOUNCE_TYPE_NULL,
		),
		CWSBM_BOUNCE_USER_REJECT => array
		(
			'remove'		=> true,
			'bounce_type'	=> CWSBM_BOUNCE_TYPE_HARD,
		),
		CWSBM_BOUNCE_WARNING => array
		(
			'remove'		=> false,
			'bounce_type'	=> CWSBM_BOUNCE_TYPE_SOFT,
		),
	...
	protected function processBodyRules($msg_no, $body, $structure, $debug=false)
	{
		$result = $this->_based_result;
		
		if ($debug) {
			echo '<h3>Msg #' . $msg_no . ' processBodyRules</h3>';
		}
		...
		*/
		...
		if ($result['rule_no'] != '0000' && $result['bounce_type'] === CWSBM_BOUNCE_TYPE_NULL) {
	...
	protected function processDSNRules($msg_no, $dsn_msg, $dsn_report, $debug=false)
	{
		if ($debug) {
			echo '<h3>Msg #' . $msg_no . ' processDSNRules</h3>';
		}
		...
		// Get the recipient email via dsn report RFC 0822 (http://www.ietf.org/rfc/rfc0822.txt)
		...
		if (empty($result['email'])) {
			...
			/* RFC 1894 Delivery Status Notifications (http://www.ietf.org/rfc/rfc1894.txt)
			 * "failed" / "delayed" / "delivered" / "relayed" / "expanded"
			 */
			switch ($action) {
				case 'failed':
					// rules based one the diag code
					$result = $this->processDSNFailedByDiagCode($result, $diag_code);
					
					// rules based one the dsn msg
					if ($result['rule_no'] == '0000') {
						$result = $this->processDSNFailedByDSNMsg($result, $dsn_msg);
					}
				break;
		...
		if ($debug) {
			echo '<strong>E-mail:</strong> ' . $result['email'] . $this->_newline;
			echo '<strong>Action:</strong> ' . $action . $this->_newline;
			echo '<strong>Status:</strong> ' . $status_code . $this->_newline;
			echo '<strong>Diagnostic-Code:</strong> ' . $diag_code . $this->_newline;
	...
	private function processDSNFailedByDiagCode($result, $diag_code)
	{
		/* rule: full
		 * sample:
		 ...
		 * sample 2:
		 * Diagnostic-Code: X-Postfix; host mta5.us4.domain.com.int[111.111.111.111] said: 552 recipient storage full, try again later (in reply to RCPT TO command)
		 * sample 3:
		 * Diagnostic-Code: X-HERMES; host 127.0.0.1[127.0.0.1] said: 551 bounce as<the destination mailbox <xxxxx@yourdomain.com> is full> queue as 100.1.ZmxEL.720k.1140313037.xxxxx@yourdomain.com (in reply to end of DATA command)
		 */
		...
		/* rule: full
		 * sample 1:
		 * Diagnostic-Code: X-Postfix; cannot append message to destination file /var/mail/dale.me89g: error writing message: File too large
		 * sample 2:
		 * Diagnostic-Code: X-Postfix; cannot access mailbox /var/spool/mail/b8843022 for user xxxxx. error writing message: File too large
		 */
		...
		/* rule: unknown
		 * sample:
		 * Diagnostic-Code: X-Postfix; host m2w-in1.domain.com[111.111.111.000] said: 551 <xxxxx@yourdomain.com> is a deactivated mailbox (in reply to RCPT TO command)
		 */
		...
		/* rule: unknown
		 * sample:
		 * Diagnostic-Code: SMTP; 550 <xxxxx@yourdomain.com> recipient rejected ... <<< 550 <xxxxx@yourdomain.com> recipient rejected 550 5.1.1 xxxxx@yourdomain.com... User unknown
		 */
		...
		/* rule: command_reject
		 * sample:
		 * Diagnostic-Code: SMTP; 550-ds176.domain.com [111.111.111.211] is currently not permitted to relay through this server. Perhaps you have not logged into the pop/imap server in the last 30 minutes or do not have SMTP Authentication turned on in your email client.
		 */
		...
		/* rule: dns_unknown
		 * sample:
		 * Diagnostic-Code: X-Postfix; delivery temporarily suspended: connect to 111.111.11.112[111.111.11.112]: No route to host
		 */
		...
		/* rule: delayed
		 * sample:
		 * Diagnostic-Code: X-Postfix; delivery temporarily suspended: conversation with 111.111.111.11[111.111.111.11] timed out while sending end of data -- message may be sent more than once
		 */
	...
	private function processDSNFailedByDSNMsg($result, $dsn_msg)
	{
		/* rule: unknown
		 * sample:
		 ...
		 */
		if (preg_match('#(?:alias|account|recipient|address|email|mailbox|user)(.*)invalid#i', $dsn_msg)) {
			$result['rule_cat']    = 'unknown';
			$result['rule_no']     = '0107';
		}
